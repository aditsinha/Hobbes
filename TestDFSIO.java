...
public class TestDFSIO extends TestCase implements Tool {
  ...
  public void testIOs(int fileSize, int nrFiles)
    ...
    try {
	  ...
	  ModifiedFileSystem fs = ModifiedFileSystem.get(config);
  ...
  private void createControlFile(ModifiedFileSystem fs,
    ...
    for(int i=0; i < nrFiles; i++) {
      ...
      try {
	  	FileSystem raw_fs = fs.getRawFileSystem();
        writer = SequenceFile.createWriter(raw_fs, config, controlFile,
  ...
  private void writeTest(ModifiedFileSystem fs) throws IOException {
    Path writeDir = getWriteDir(config);
    fs.delete(getDataDir(config));
    fs.delete(writeDir);
  ...
  private void appendTest(ModifiedFileSystem fs) throws IOException {
  ...
  private void readTest(ModifiedFileSystem fs) throws IOException {
    Path readDir = getReadDir(config);
    fs.delete(readDir);
    runIOTest(ReadMapper.class, readDir);
  }

  private void sequentialTest(ModifiedFileSystem fs, 
                              int testType, 
                              long fileSize, // in bytes
                              int nrFiles
                             ) throws IOException {
  ...
  private void analyzeResult( ModifiedFileSystem fs,
                              int testType,
                              long execTime,
                              String resFileName
                            ) throws IOException {
    ...
    try {
	  FileSystem raw_fs = fs.getRawFileSystem();
      in = new DataInputStream(raw_fs.open(reduceFile));
  ...
  private void cleanup(ModifiedFileSystem fs)
  throws IOException {
    LOG.info("Cleaning up test files");
    fs.delete(new Path(getBaseDir(config)));
  }
}
